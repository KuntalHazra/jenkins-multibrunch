        ...
        stage('Merge Changes') {
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: env.GIT_CREDENTIALS_ID, passwordVariable: 'GIT_PASSWORD',passwordVariabl: 'GIT_USERNAME')]) {
                        sh '''
                            # Use the credentials to set the remote URL and push changes
                            git remote set-url origin https://passwordVariabl:passwordVariabl@github.com/KuntalHazra/jenkins-multibranch.git
